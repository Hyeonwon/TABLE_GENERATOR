...
void generateSchedule() {
    ...
    cout << "\n시간표 생성이 완료되었습니다.\n";
    displayScheduleTable(generatedSchedule);
    waitForEnterOrEsc();
}
...
void modifySchedule() {
    try {
        ...
        vector<int> foundSchedules = querySchedules(userName, year, semester, scheduleID);
        ...
        cout << "\n검색된 시간표 목록:" << endl;
        int index = 1;
        for (int id : foundSchedules) {
            Table table = getTableById(id);
            cout << index << ") 시간표 ID: " << table.get_id()
                 << ", 학년: " << table.get_year()
                 << ", 학기: " << encode_semester(table.get_semester())
                 << ", 사용자 이름: " << current_user.name << endl;
            index++;
        }

        int selectedScheduleIndex = navigateSchedules(foundSchedules);
        if (selectedScheduleIndex == -1) {
            return; 
        }

        modifySelectedSchedule(foundSchedules[selectedScheduleIndex]);
    } catch (runtime_error&) {
...
void addLectureToSchedule(Table& schedule) {
    while (true) {
        ...
        Table tempTable(-1, -1, "", foundLectures, schedule.get_year(), schedule.get_semester());
        displayLectureList(tempTable);
...
void displayLectureList(const Table& table) {
    ...
    for (size_t i = 0; i < courses.size(); ++i) {
        ...
        cout << setw(4) << (i + 1) << " | "
             << setw(12) << course.get_name() << " | "
             << setw(10) << course.get_professor() << " | "
             << setw(4) << course.get_grade() << " | ";
        for (const auto& time : course.get_times()) {
